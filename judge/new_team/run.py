...
def test(user_id,net,video):


    sys.path.append(u'/root/mmgc/team/' + user_id + u'/submit/')
    print(sys.path)
    import ABR
    ...
    network_trace_dir = '/root/mmgc/dataset/test_network_trace/' + NETWORK_TRACE + '/'
    video_trace_prefix = '/root/mmgc/dataset/test_video/' + VIDEO_TRACE + '/frame_trace_'
...
#result = test(user_id)
final_results = {}
final_results['time'] = 0
net_list=['high','medium','low','fixed']
video_list = ['Dota','erke','WorldCup']
'''
for net in net_list:
    tmp_score = []
    tmp_time = []
    for video in video_list:
        print(net,video)
        result = test(user_id,net,video)
        tmp_score.append(result[0])
        tmp_time.append(result[1])
    final_results[net] = np.mean(tmp_score)
    final_results['time'] += np.mean(tmp_time) 
final_results['time'] = final_results['time'] / 4  
'''
try:
    for net in net_list:
        tmp_score = []
    ...
    sql = "INSERT INTO mmgc_final  VALUES ( '%s' , '%s' ,'%s', %s,%s,%s,%s,%s,'import file error');"%(user_id, file_name,time, 0,0,0,0,0)
    try:
        cursor.execute(sql)
        print("Import Error")
        db.commit()
    ...
    sql = "INSERT INTO mmgc_final  VALUES ('%s' , '%s','%s', %s,%s,%s,%s,%s,'run programmer Syntax error');"%(user_id,file_name,time,0,0,0,0, 0)
    try:
         cursor.execute(sql)
         print("Synatax Error")
         db.commit()
    ...
    sql = "INSERT INTO mmgc_final  VALUES ('%s' , '%s','%s',%s, %s,%s,%s,%s,' unkown error');"%(user_id, file_name,time,0, 0,0,0,0)
    try:
         cursor.execute(sql)
         print("Unkown Error")
         db.commit()
    ...
    sql = "INSERT INTO mmgc_final  VALUES ( '%s' ,  '%s' ,'%s', %s,%s,%s,%s,%s,'NULL ');"%(user_id,file_name, time, final_results['high'],final_results['medium'],final_results['low'],final_results['fixed'],final_results['time'])
    try:
        cursor.execute(sql)
        db.commit()
        print("success")
